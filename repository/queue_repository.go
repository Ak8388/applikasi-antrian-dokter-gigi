...
type QueueRepo interface {
	...
	CountDataReservasi(id, date, time string) int32
...
func (q *queueRepo) Reschedule(data model.Queue) (err error) {
	qry := "Update queues Set queue_date=$1, queue_time=$2, queue_number=$3,status=$4 Where id=$5"

	_, err = q.db.Exec(qry, data.QueueDate, data.QueueTime, data.QueueNumber, data.Status, data.ID)

	return
}

func (q *queueRepo) CreateQueue(data model.Queue) (model.Queue, error) {
	qry := "Insert Into queues (doctor_id, patient_id, queue_date, queue_time, queue_number, note,status) Values($1,$2,$3,$4,$5,$6,$7) Returning created_at,updated_at"
	tx, err := q.db.Begin()
	...
	err = tx.QueryRow(qry, data.Doctor, data.Patient, data.QueueDate, data.QueueTime, data.QueueNumber, data.Note, data.Status).Scan(&data.CreatedAt, &data.UpdateAt)
...
func (q *queueRepo) GetQueueByPatientId(patientId, status string) (data []model.Queue, err error) {
	...
	for rows.Next() {
		queue := model.Queue{}
		err = rows.Scan(&queue.ID, &queue.Doctor, &queue.Patient, &queue.QueueDate, &queue.QueueTime, &queue.QueueNumber, &queue.Note, &queue.Status, &queue.CreatedAt, &queue.UpdateAt)
...
func (q *queueRepo) CountDataReservasi(id, date, time string) (totalData int32) {
	sql := "Select COUNT(id) as total_data From queues Where doctor_id=$1 AND queue_date=$2 AND queue_time=$3 AND status=$4 OR status=$5"

	err := q.db.QueryRow(sql, id, date, time, "created", "reschedule").Scan(&totalData)
