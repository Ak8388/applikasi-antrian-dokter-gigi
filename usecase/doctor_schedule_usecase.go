...
type DoctorScheduleUsecase interface {
	...
	FindScheduleID(day, openingHours string) (id string, err error)
}
...
func (ds *doctorScheduleUsecase) UpdateNewSchedule(scheduleDto dto.ScheduleDoctors) (model.ScheduleDoctors, error) {
	...
	return ds.dsRepo.UpdateSchedule(schedule)
}
...
func (ds *doctorScheduleUsecase) FindScheduleID(day, openingHours string) (id string, err error) {
	if day == "" || openingHours == "" {
		return "", errors.New("day and opening hours can't be empty")
	}

	opHours := strings.Replace(openingHours, " +0000 UTC", "", -1)

	return ds.dsRepo.SelectScheduleID(day, opHours)
}
